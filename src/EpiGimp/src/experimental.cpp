...
static auto drawCircle = [](const sf::Vector2f &pt, sf::Image &img) -> void {
    ...
    for (auto y = -RADIUS; y != RADIUS; y++)
        for (auto x = -RADIUS; x != RADIUS; x++)
            if (pt.x + x >= 0 && pt.x + x <= img.getSize().x && pt.y + y >= 0 && pt.y + y <= img.getSize().y
...
auto main() -> int
{
    ...
    sf::RenderWindow w{{WINDOW_W, WINDOW_H}, "Experimental Gimp Application", sf::Style::Close | sf::Style::Resize};
    ...
    w.setFramerateLimit(FPS_LIMIT);
    w.setVerticalSyncEnabled(true);

    w.setKeyRepeatEnabled(true);

    constexpr auto CANVAS_W = 1080;
    constexpr auto CANVAS_H = 720;
    const auto DEFAULT_CANVAS_COLOR = sf::Color::White;

    sf::Image drawing;
    drawing.create(CANVAS_W, CANVAS_H, DEFAULT_CANVAS_COLOR);
    sf::Texture canvasTexture;
    canvasTexture.loadFromImage(drawing);

    sf::RectangleShape canvas{{CANVAS_W, CANVAS_H}};
